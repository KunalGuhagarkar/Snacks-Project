...
export default function CheckoutPage({ cartItems: propCartItems, currentUser, setCartItems }) {
  ...
  const cartItems = useMemo(() => {
    ...
    if (location.state?.cartItems && location.state.cartItems.length > 0) return location.state.cartItems;
    return [];
  }, [dbCartItems, propCartItems, location.state]);

  const subtotal = useMemo(() => {
    return cartItems.reduce((acc, item) => acc + Number(item.price || item.historical_price || 0) * item.quantity, 0);
  }, [cartItems]);
  ...
  const handlePlaceOrder = async (e) => {
    ...
    if (cartItems.length === 0) {
      alert("🛒 Your checkout cart is empty! Add products before checking out.");
      return;
    }
    ...
    try {
      // ✅ Explicit target checkpoint resolution fallback if configuration mapping string varies
      const targetUrl = API_ENDPOINTS.submitOrder || "http://localhost:5000/api/orders/checkout";
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to commit order execution pipeline.");
      }

      alert(`🎉 Order Placed Successfully! Your Order Reference ID is #${data.orderId}`);
  ...
  const labelStyle = { fontSize: "0.85rem", fontWeight: "600", color: "#7a6a54" };
  const inputStyle = { width: "100%", padding: "12px", margin: "6px 0 14px 0", borderRadius: 8, border: "1px solid #bfb09a", background: "#fdfaf4", fontSize: "0.95rem", boxSizing: "border-box", color: "#1c1409" };
    ...
    <div style={{ minHeight: "100vh", background: "#fdfaf4", padding: "40px 20px", fontFamily: "sans-serif" }}>
      <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "30px", borderBottom: "1px solid #ede3d0", paddingBottom: "16px" }}>
          ...
          <button onClick={() => navigate("/")} disabled={loading} style={{ border: "none", background: "#f5efe1", padding: "10px 18px", borderRadius: "8px", cursor: "pointer", fontWeight: "600", color: "#1c1409" }}>
            ← Keep Shopping
          </button>
        </div>

        {cartItems.length === 0 ? (
          <div style={{ background: "white", padding: "50px", textAlign: "center", borderRadius: "12px", border: "1px solid #ede3d0" }}>
            <h2>Your basket appears to be empty 📦</h2>
            <button onClick={() => navigate("/")} style={{ background: "#e07b2a", color: "white", border: "none", padding: "12px 24px", borderRadius: "8px", fontWeight: "700", cursor: "pointer", marginTop: "15px" }}>
              ...
              <form onSubmit={handlePlaceOrder}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                  <div>
                    ...
                    <input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} required style={inputStyle} placeholder="e.g. Lakshmi Raman" disabled={loading} />
                  </div>
                  <div>
                    <label style={labelStyle}>Mobile Phone Number *</label>
                    <input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} required style={inputStyle} placeholder="e.g. 9845012345" disabled={loading} />
                  </div>
                </div>

                <div style={{ marginTop: "6px" }}>
                  <label style={labelStyle}>Street Address / Door No. / Building *</label>
                  <input type="text" name="streetAddress" value={formData.streetAddress} onChange={handleInputChange} required style={inputStyle} placeholder="e.g. #42, 3rd Main, Gokulam" disabled={loading} />
                </div>

                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", marginTop: "6px" }}>
                  <div>
                    <label style={labelStyle}>City *</label>
                    <input type="text" name="city" value={formData.city} onChange={handleInputChange} required style={inputStyle} placeholder="Mysuru" disabled={loading} />
                  </div>
                  <div>
                    <label style={labelStyle}>State *</label>
                    <input type="text" name="stateName" value={formData.stateName} onChange={handleInputChange} required style={inputStyle} placeholder="Karnataka" disabled={loading} />
                  </div>
                  <div>
                    <label style={labelStyle}>Pincode / Zip *</label>
                    <input type="text" name="pincode" value={formData.pincode} onChange={handleInputChange} required style={inputStyle} placeholder="570002" disabled={loading} />
                  </div>
                </div>
                ...
                <button type="submit" disabled={loading} style={{ width: "100%", background: loading ? "#7a6a54" : "#b5591a", color: "white", border: "none", padding: "16px", borderRadius: "8px", fontSize: "1.1rem", fontWeight: "700", cursor: loading ? "not-allowed" : "pointer", marginTop: "16px" }}>
                  {loading ? "🔄 Processing Secured Order..." : `Complete Order & Pay ₹${grandTotal} →`}
                </button>
              </form>
            </div>
